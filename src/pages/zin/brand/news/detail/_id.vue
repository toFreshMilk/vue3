        ...
        <article class="divider_cnt">
          ...
          <div class="page_body news_detail_body type_default">
            <div class="detail_head">
              ...
              <div class="btn_wrap">
                <div class="print_btn_area btn_area">
                  <div class="print_btn" role="button" @click="printNew()">
            ...
            <div class="detail_body">
              <!-- <div class="img_wrap">
                <img src="/img/zin/@_detail_02.png" alt="상세이미지" />
              </div> -->
...
export default {
  ...
  async fetch() {
    // this.mypage = this.$nuxt.route.query.page;
  ...
  methods: {
    ...
    async newsdetail() {
      try {
        const res = await this.$nuxt.$fetch({
          api: {
            path: "/svc/newsletterSvcApi/svcRead",
            method: "get",
          },
          query: {
            _siteId: "zinoct",
        ...
        this.detailinfo = listdata;

        if (this.detailinfo.title.length > 30) {
          this.detailinfo.title =
            this.detailinfo.title.substr(0, 30) +
            `\n` +
            this.detailinfo.title.substr(30, this.detailinfo.title.length);
        }
        if (this.detailinfo.prvTitle.length > 30) {
          this.detailinfo.prvTitle =
            this.detailinfo.prvTitle.substr(0, 30) + "...";
        }
        if (this.detailinfo.nxtTitle.length > 30) {
          this.detailinfo.nxtTitle =
            this.detailinfo.nxtTitle.substr(0, 30) + "...";
        }
      } catch (e) {
        console.log(e);
      }
    ...
    detail(v) {
      this.$router.push({ path: "../detail/" + v ,
      query:{
        page : this.mypage
