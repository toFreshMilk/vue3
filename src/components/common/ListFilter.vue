  ...
  <div class="filter_wrap">
    <div class="filter_head">
      ...
      <div class="sel_filter_area">
        <div class="chip_ls">
          <v-chip v-for="(chip, k) in selChips" :key="'chip_'+k"
                  close @click:close="removeChip(k)" class="chip_item">
    ...
    <div class="filter_body">
      ...
      <div v-for="(filterCate, i) in filtersData"
           v-if="selectedFilter.state && selectedFilter.name === filterCate.name"
        ...
        <div v-if="filterCate.type === 'checkbox' && filterCate.name === 'color'" class="input_area check inline type_color">
          <div v-for="(filter, k) in filterCate.opts" :key="'detail_filter' + i + '_'+ k"
            ...
            <label v-if="filter.value === 'fff' || filter.value === 'ffffff' || filter.label === 'white' || filter.label === '흰색'"
                   class="c_white" @change="setColor(filter.value)"
            >
            ...
            <label v-else-if="filter.value === 'all' || filter.label === 'all' || filter.label === '전체'"
                   class="c_all" @change="setColor(filter.value)"
            >
              <input v-model="selColor" type="checkbox" :name="filter.name" :value="filter.value">
              <span>{{ filter.label }}</span>
            </label>
            <label v-else-if="filter.value === 'silver' || filter.label === 'silver' || filter.label === '실버'"
                   class="c_silver" @change="setColor(filter.value)"
            >
              <input v-model="selColor" type="checkbox" :name="filter.name" :value="filter.value">
              <span>{{ filter.label }}</span>
            </label>
            <label v-else-if="filter.value === 'gold' || filter.label === 'gold' || filter.label === '골드'"
                   class="c_gold" @change="setColor(filter.value)"
            >
              <input v-model="selColor" type="checkbox" :name="filter.name" :value="filter.value">
              <span>{{ filter.label }}</span>
            </label>
            <label v-else  @change="setColor(filter.value)">
              <input v-model="selColor" type="checkbox" :name="filter.name" :value="filter.value">
        ...
        <div v-else-if="filterCate.type === 'radio'" class="input_area radio type_chk inline">
          <div class="input_box" v-for="(filter, k) in filterCate.opts" :key="'detail_filter' + i + '_'+ k">
            <label @change="setRadio(filter.label, filterCate.name)">
              <input v-model="selRadio.current" type="radio" :name="filter.name" :value="filter.label">
              <span>{{ filter.label }}</span>
            </label>
          </div>
        </div>
        <div v-else-if="filterCate.type === 'checkbox'" class="input_area check inline">
          <div class="input_box" v-for="(filter, k) in filterCate.opts" :key="'detail_filter' + i + '_'+ k">
            <label>
              <input v-model="selChips" type="checkbox" :name="filter.name" :value="filter.label">
              <span>{{ filter.label }}</span>
            </label>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    setRadio(val, name) {
      ...
      if(num !== -1) this.selChips.splice(num, 1);
      this.selChips.push(val)
      this.selRadio[name].before = val;
    },
    setColor(val) {
      let c = 'c_' + val;
      let num = this.selChips.indexOf(c);
      if(num !== -1 && this.selColor.indexOf(val) === -1) {
        this.selChips.splice(num, 1);
        return
      }
