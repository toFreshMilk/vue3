  ...
  <div class="snb_wrap zin_snb_wrap">
              ...
              <ul v-if="dep2.dep3" v-show="activeMenu.name.dep2 === dep2.name && activeMenu.state.dep2"
                ...
                <li v-for="(dep3, k) in dep2.dep3" :key="'snb_dep_'+i+'_'+j+'_'+k" class="snb_node">
                  ...
                  <button v-if="dep3.dep4" @click="foldMenu(dep3.name, 3)" class="fold_btn"
                          :class="{'is_active': activeMenu.name.dep3 === dep3.name && activeMenu.state.dep3}">
    ...
    <div v-if="BannerShow" class="banner_menu_wrap">
      <div class="banner_menu search">
        <NuxtLink to="/publish/pc/zin/product/" class="banner_menu_item"><span>Z:IN 제품검색</span></NuxtLink>
      </div>
